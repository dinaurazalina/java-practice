...
public class Book {
    ...
    public static Boolean isTitleWord(String word) {
    	boolean bo = true;
    	String [] nono = { "a" , "an", "the", "and", "but", "for", "nor", "or", "so", "yet", "at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};
    	for (int i = 0; i < nono.length; i++) {

    	if(word.equalsIgnoreCase(nono[i])) {
    		bo = false;
    	}
    		}
